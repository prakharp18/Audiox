...
export default function Home() {
  ...
  return (
    <main className="relative min-h-screen w-full bg-[#f9f7f2] text-zinc-900 overflow-hidden selection:bg-primary selection:text-white">
      ...
      <div 
        className="absolute top-4 right-4 sm:top-6 sm:right-6 z-50 flex flex-col items-end"
      ...
      >
        <Link href="https://vox-omega.vercel.app/" target="_blank">
          <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2 rounded-2xl bg-white/80 border border-zinc-200 backdrop-blur-md cursor-pointer hover:bg-white hover:border-zinc-300 transition-all duration-300 shadow-sm">
              ...
              <span className="text-lg sm:text-xl font-heading font-black tracking-tighter text-zinc-900">VOX</span>
        ...
        <AnimatePresence>
            {isLogoHovered && (
                <motion.div
                    ...
                    className="mt-3 w-[calc(100vw-2rem)] sm:w-80 p-0 rounded-3xl bg-white/95 border border-zinc-200 backdrop-blur-xl shadow-2xl origin-top-right overflow-hidden group hidden sm:block"
                >
      ...
      {/* Hero Section */}
      <div className="relative z-10 flex flex-col items-center justify-start min-h-screen p-4 pt-24 sm:pt-32 text-center md:pt-40">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 1.1, delay: 0.2, ease: "easeOut" }}
          className="flex flex-col items-center gap-4 sm:gap-6 max-w-2xl px-2 sm:px-4"
        >
          {/* Status Badge */}
          ...
          {/* Heading - natural wrap on mobile, forced break on desktop */}
          <h1 className="text-4xl sm:text-5xl md:text-7xl font-heading font-bold tracking-tight text-zinc-900 leading-[0.9]">
            Share what you{" "}
          ...
          {/* Subtitle */}
          <p className="text-zinc-600 text-sm md:text-base max-w-xs sm:max-w-md leading-relaxed font-medium">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 mt-2 sm:mt-4 w-full sm:w-auto">
            <Link href="/login" className="w-full sm:w-auto">
                <button className="group relative w-full sm:w-auto flex items-center justify-center gap-3 px-6 sm:px-8 py-0 h-14 sm:h-16 bg-zinc-950 text-white rounded-full font-medium transition-all hover:bg-zinc-900 hover:scale-105 active:scale-95 shadow-xl hover:shadow-2xl text-sm sm:text-base whitespace-nowrap">
